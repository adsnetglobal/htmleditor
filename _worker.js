...
function withSecurityHeaders(response) {
  try {
    ...
    const base = securityHeadersBase();
    Object.keys(base).forEach((key) => {
      if (!headers.has(key)) headers.set(key, base[key]);
    });
...
function resolveStaticRoute(pathname) {
  ...
  if (p === '/layanan') {
    return { redirectPath: '/services.html', status: 301 };
  }

  if (p.startsWith('/layanan/')) {
    return { rewritePath: '/service-detail.html' };
  }

  if (p.startsWith('/blog/')) {
    return { rewritePath: '/article.html' };
  }
...
function isHtmlNavigationRequest(request, pathname) {
  ...
  if (p.startsWith('/api') || p.startsWith('/webhook') || p === '/health' || p.startsWith('/__worker_')) {
    return false;
  }

  if (hasFileExtension(p)) return false;
...
async function fetchStaticAssetWithFallback(assetRequest, originalRequest, env, assetPathname) {
  const res = await env.ASSETS.fetch(assetRequest);
  if (res.status !== 404) {
    return res;
  }

  if (!isHtmlNavigationRequest(originalRequest, assetPathname)) {
    return res;
  }
  ...
  indexUrl.search = '';
  const indexRequest = new Request(indexUrl.toString(), originalRequest);
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/__worker_metrics') {
      if (request.method !== 'GET') return withSecurityHeaders(new Response('Method Not Allowed', { status: 405 }));
      if (!metricsAuthOk(request, env)) return withSecurityHeaders(new Response('Unauthorized', { status: 401 }));
    ...
    if (_blockedPaths.has(url.pathname) || _blockedPaths.has(pLower)) {
      return withSecurityHeaders(new Response('Not Found', { status: 404, headers: { 'Content-Type': 'text/plain; charset=utf-8', 'Cache-Control': 'no-store' } }));
    }
    ...
    if (url.pathname === '/health') {
      ...
      return withSecurityHeaders(await handleHealth(env, ctx));
    }
    ...
    if (url.pathname === '/webhook/moota') {
      ...
      return withSecurityHeaders(await handleWebhook(request, env.MOOTA_GAS_URL, env.MOOTA_TOKEN));
    }

    const staticRoute = resolveStaticRoute(url.pathname);
    if (staticRoute.redirectPath) {
      const redirectUrl = new URL(request.url);
      redirectUrl.pathname = staticRoute.redirectPath;
      const headers = {
        Location: redirectUrl.toString(),
        'Cache-Control': 'public, max-age=300'
      };
      return withSecurityHeaders(new Response(null, { status: staticRoute.status || 302, headers }));
    }

    const assetPathname = staticRoute.rewritePath || url.pathname;
    const routedUrl = new URL(request.url);
    routedUrl.pathname = assetPathname;
    const routedRequest = new Request(routedUrl.toString(), request);

    if (request.method === 'GET') {
      const cacheable = isCacheableAssetPath(assetPathname);
      if (cacheable) {
        const cacheKey = new Request(routedUrl.toString(), { method: 'GET' });
        const cached = await caches.default.match(cacheKey);
        if (cached) {
          inc('assets_cache_hit');
          return withSecurityHeaders(maybeCompress(request, withStaticCacheHeaders(withMetricHeaders(cached, { 'x-edge-cache': 'HIT' }), assetPathname)));
        }
        inc('assets_cache_miss');
        try {
          if (env.ASSETS) {
            const res = await env.ASSETS.fetch(routedRequest);
            const withHeaders = withStaticCacheHeaders(withMetricHeaders(res, { 'x-edge-cache': 'MISS' }), assetPathname);
            if (res.ok) ctx.waitUntil(caches.default.put(cacheKey, withHeaders.clone()));
            return withSecurityHeaders(maybeCompress(request, withHeaders));
          }
        } catch (e) { }
      }
    }
    ...
    try {
      if (env.ASSETS) {
        const staticRes = await fetchStaticAssetWithFallback(routedRequest, request, env, assetPathname);
        return withSecurityHeaders(maybeCompress(request, withStaticCacheHeaders(staticRes, assetPathname)));
      }
    } catch (e) {
    ...
    return withSecurityHeaders(maybeCompress(request, withStaticCacheHeaders(await fetch(request), url.pathname)));
  }
};
...
async function handleApi(request, env, ctx) {
  ...
  try {
    ...
    const actionName = parsedBody && typeof parsedBody.action === 'string' ? String(parsedBody.action) : '';
    ...
    const sensitiveActions = new Set(['book_appointment', 'subscribe_newsletter', 'confirm_newsletter', 'forgot_password', 'login', 'login_and_dashboard']);
    const sensitiveLimit = Number(env.SENSITIVE_API_RPM_LIMIT || 12);
    if (sensitiveLimit > 0 && sensitiveActions.has(actionName)) {
      const sensitiveOk = softRateLimitOk(request, sensitiveLimit);
      if (!sensitiveOk) {
        inc('rate_limited');
        return new Response(JSON.stringify({ status: 'error', message: 'Rate limited for sensitive action' }), {
          status: 429,
          headers: { 'Content-Type': 'application/json', ...corsHeadersFor(request, env), 'Retry-After': '60' }
        });
      }
    }
